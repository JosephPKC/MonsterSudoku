...
/*
 * void Puzzle::displayUpdate(std::ostream& out, const Position& special) const {
	HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
	for(std::size_t x = 0; x < getSize(); ++x) {
		if(x % getP() == 0) {
			for(std::size_t i = 0; i < (std::size_t)(2 * getN() + 2 * getP() + 1); ++i) {
				out << "-";
			}
			out << std::endl;
		}
		for(std::size_t y = 0; y < getSize(); ++y) {
			...
			try {
				if(Position(x, y) == special) {
					std::size_t val = access(x, y).getValue();
					if(val == getSize()) {
...
std::ostream& operator << (std::ostream& out, const Puzzle& p) {
	try {
		p.displayUpdate(out, Position(p.getSize(), p.getSize()));
	}
	catch(std::out_of_range e) {
		return out;
	}
